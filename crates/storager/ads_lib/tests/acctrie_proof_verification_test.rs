...
use ads_rust::acctrie::trie::AccTrie;
...
fn test_insertion_proof_verification() {
    ...
    assert!(
        proof3.keyp_in_ln_proof.is_some(),
        "Should have keyp membership proof in LN"
    );
    assert!(
        proof3.keyp_in_ln_next_old_proof.is_some(),
        "Should have keyp membership proof in old LNn"
    );
    assert!(
        proof3.key_in_ln_next_new_proof.is_some(),
        "Should have key membership proof in new LNn"
    );
    assert!(
        proof3.value_in_ln_proof.is_some(),
        "Should have value membership proof"
    );
}
...
fn test_deletion_proof_verification() {
    ...
    assert!(
        del_proof.keyp_in_ln_proof.is_some(),
        "Should have keyp membership proof in LN"
    );
    assert!(
        del_proof.key_in_ln_next_old_proof.is_some(),
        "Should have key membership proof in old LNn"
    );
    assert!(
        del_proof.keyp_in_ln_next_new_proof.is_some(),
        "Should have keyp membership proof in new LNn"
    );
}
...
fn test_partial_deletion_proof_verification() {
    ...
    // 验证证明包含必要的成员证明
    assert!(
        !del_proof.delete_entire_leaf,
        "Should not delete entire leaf"
    );
    assert!(
        del_proof.value_in_ln_old_proof.is_some(),
        "Should have value membership proof in old LN"
    );
    assert!(
        del_proof.ln_acc_new.is_some(),
        "Should have new accumulator value"
    );
}
...
fn test_update_proof_verification() {
    ...
    // 验证证明包含必要的证明
    assert!(
        update_proof.delete_value_proof.is_some(),
        "Should have delete proof"
    );
    assert!(
        update_proof.add_value_proof.is_some(),
        "Should have add proof"
    );
...
fn test_query_existence_proof() {
    ...
    match query_result {
        ads_rust::acctrie::trie::QueryResult::Exists(proof) => {
            ...
            assert!(
                proof.membership_proof.is_some(),
                "Should have membership proof"
            );
        }
...
fn test_query_non_existence_proof() {
    ...
    assert!(
        audit_result.valid,
        "Query non-existence proof should be valid"
    );
    ...
    match query_result {
        ads_rust::acctrie::trie::QueryResult::NotExists(proof) => {
            ...
            assert!(
                proof.prev_in_next_proof.is_some(),
                "Should have prev membership proof"
            );
        }
...
fn test_complete_workflow_with_proofs() {
    ...
    for (key, value) in &keys {
        ...
        assert!(
            result.valid,
            "Insertion proof should be valid for key {:?}",
            key
        );
    }
    ...
    for (key, value) in &keys {
        ...
        assert!(
            audit_result.valid,
            "Query proof should be valid for key {:?}",
            key
        );
    }
