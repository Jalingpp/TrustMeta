...
pub static EMPTY_HASH: LazyLock<Hash> = LazyLock::new(|| leaf_hash("", ""));

pub static EMPTY_ACC: LazyLock<G1Affine> = LazyLock::new(|| DynamicAccumulator::empty_commitment());
...
pub fn nonleaf_hash(left: Hash, right: Hash, keys: &Set<String>, acc: &G1Affine) -> Hash {
    ...
    acc.serialize(&mut acc_bytes)
        .expect("failed to serialize acc");
