...
pub struct SiblingNonMembershipProof {
    ...
    pub sibling_accumulator: G1Affine,
...
pub struct PathElement {
    ...
    pub sibling_keys: Set<String>,
    #[serde(with = "serde_impl")]
    pub sibling_acc: G1Affine,
    pub parent_keys: Set<String>,
    #[serde(with = "serde_impl")]
    pub parent_acc: G1Affine,
}
...
impl SelectResult {
    ...
    pub fn verify(&self, expected_key: &str) -> bool {
        ...
        self.results.iter().all(|result| {
            let leaf = crate::utils::leaf_hash(expected_key, &result.fid);
            result.tree_proof.leaf_merkle_proof.verify(leaf)
                && result.tree_proof.root_membership_proof.verify(
                    &result.tree_proof.root_membership_proof.accumulator,
                    expected_key,
                )
...
impl InsertResult {
    ...
    pub fn verify_insert(&self, expected_key: &str, expect_fid: &str) -> bool {
        if self.fid != expect_fid {
            return false;
        }
        ...
        let leaf = crate::utils::leaf_hash(expected_key, &self.fid);
        self.post_merkle_proof.verify(leaf)
            && self
                .post_acc_proof
...
impl DeleteResult {
    ...
    pub fn verify_delete(&self, expected_key: &str) -> bool {
        ...
        if let Some(pre_merkle) = &self.pre_merkle_proof {
            ...
            if !pre_merkle.verify(crate::utils::leaf_hash(expected_key, old_fid)) {
                return false;
            }
        }
        ...
        if let Some(post_merkle) = &self.post_merkle_proof {
            ...
            if !post_merkle.verify(crate::utils::leaf_hash(expected_key, new_fid)) {
                return false;
            }
