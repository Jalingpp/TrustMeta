...
impl SEH {
    ...
    pub fn serialize_seh(&self) -> Result<String, serde_json::Error> {
        ...
        let keys: Vec<String> = self.ht.read().unwrap().keys().cloned().collect();
        let se = SeSEH {
            gd: self.gd,
            rdx: self.rdx,
...
mod tests {
    ...
    fn test_seh_deep_splits_distribution_rdx16() {
        ...
        for (dir_key, bkt) in seh.read().unwrap().ht.read().unwrap().iter() {
            ...
            println!(
                "[seh] bucket dir_key='{}' size={} (cap={})",
                dir_key, size, capacity
            );
            assert!(
                size <= capacity,
                "bucket '{}' size {} exceeds capacity {}",
                dir_key,
                size,
                capacity
            );
    ...
    fn test_delete_via_seh_updates_bucket() {
        ...
        let _ = seh
            .write()
            .unwrap()
            .insert_kvpair(KVPair::new("k".into(), "v1".into()));
        let _ = seh
            .write()
            .unwrap()
            .insert_kvpair(KVPair::new("k".into(), "v2".into()));
        assert_eq!(
            seh.read()
                .unwrap()
                .get_bucket_by_key("k")
    ...
    fn test_insert_same_key_appends_value_via_seh() {
        ...
        let _ = seh
            .write()
            .unwrap()
            .insert_kvpair(KVPair::new("k".into(), "v3".into()));
        let b = seh.read().unwrap().get_bucket_by_key("k").unwrap();
        assert_eq!(
            b.read().unwrap().get_value("k").as_deref(),
            Some("v1,v2,v3")
        );
    }
}
