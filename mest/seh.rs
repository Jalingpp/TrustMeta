...
use crate::merkletree::{MHTProof, verify_proof};
...
fn make_prefix(key: &str, rdx: i32, gd: usize) -> String {
    ...
    let digits = crate::util::digits_prefix_mixed(key, rdx, gd);
...
mod tests {
    ...
    fn test_key_to_bucket_key_relation() {
        ...
        for k in &keys {
            ...
            let bk_len = bk.len();
            let dir_digits = crate::util::digits_prefix_mixed(k, seh.read().unwrap().rdx, bk_len);
