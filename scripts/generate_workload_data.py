...
def generate_keywords(category: str, num_keywords: int = None) -> list:
    ...
    if num_keywords is None:
        ...
        num_keywords = random.choices(
            [2, 3, 4, 5, 6, 7], weights=[30, 35, 20, 10, 3, 2]
        )[0]
...
def generate_dataset(size: int, output_file: str):
    ...
    stats = {
        ...
        "top_keywords": sorted(
            keyword_distribution.items(), key=lambda x: x[1], reverse=True
        )[:20],
    }
...
def main():
    ...
    parser.add_argument(
        "--custom-size", type=int, default=None, help="自定义数据集大小"
    )
